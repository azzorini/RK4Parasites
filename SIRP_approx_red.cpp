...
const double g = 1.0;
const double mu = 1.0; // mu = 1.0, 10.0, 100.0
...
const double landap = landa*beta;

// Derived initial conditions
const double I0 = mu/landa*P0;

const double C = beta*(P0 + landa/g*(S0 + I0) - mu/beta*std::log(S0));

// Functions
// S -> 0, I -> 1, R -> 2, P -> 3
inline double fS(const Eigen::Vector3d& y) {
	return landap/g*y[0]*(y[0] + y[1]) - mu*y[0]*std::log(y[0]) - y[0]*C;
}

inline double fI(const Eigen::Vector3d& y) {
	return -landap/g*y[0]*(y[0] + y[1]) + mu*y[0]*std::log(y[0]) + y[0]*C - g*y[1];
}
...
inline double get_P(const Eigen::Vector3d& y) {
	return  -landa/g*(y[0] + y[1]) + mu/beta*std::log(y[0]) + C/beta;
}
...
int main() {
	...
	std::ofstream fout("SIRP_approx_red_" + std::to_string(mu) + ".txt");
