...
const double g = 1.0;
const double mu = 100.0; // mu = 1.0, 10.0, 100.0
...
const double C = P0 + landa/g*(S0 + I0) - mu/beta*std::log(S0) - S0;

// Functions
// S -> 0, I -> 1, R -> 2, P -> 3
inline double fS(const Eigen::Vector3d& y) {
	return landa*beta/g*y[0]*(y[0] + y[1]) - mu*y[0]*std::log(y[0]) - beta*y[0]*y[0] - y[0]*beta*C;
}

inline double fI(const Eigen::Vector3d& y) {
	return -landa*beta/g*y[0]*(y[0] + y[1]) + mu*y[0]*std::log(y[0]) + beta*y[0]*y[0] + y[0]*beta*C - g*y[1];
}
...
inline double get_P(const Eigen::Vector3d& y) {
	return -landa/g*(y[0] + y[1]) + mu/beta*std::log(y[0]) + y[0] + C;
}
...
int main() {
	...
	std::ofstream fout("SIRP_exact_red_" + std::to_string(mu) + ".txt");
